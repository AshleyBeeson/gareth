...
describe('Testing search functions', function(){
	...
	var inputsArray;	
			...
			it('\nSimulate selecting items of low severity', function(){

				//find input boxes to uncheck via their ID and place reference to it in inputElements
				let inputElementA = null;
				let inputElementB = null;
				let inputToFindA = "mediumSeverityInput";
				let inputToFindB = "highSeverityInput";
				inputsArray.forEach((input) => {
												if(input.id == inputToFindA){inputElementA = input;}
												if(input.id == inputToFindB){inputElementB = input;}  
												});

				//Send unchecked values to checkboxes
				inputElementA.checked = false;
				TestUtils.Simulate.change(inputElementA);
				inputElementB.checked = false;
				TestUtils.Simulate.change(inputElementB);
				
				let componentReference = renderedBugList.refs.bugItemNum0;
				let elementReference = componentReference.refs.bugID;

				//Check first result matches expected data.
				expect(ReactDOM.findDOMNode(elementReference).textContent).toEqual("ID: ISSUE-00002");
	
			});	

			it('\nSimulate selecting items of medium severity', function(){

				//find input boxes to uncheck via their ID and place reference to it in inputElements
				let inputElementA = null;
				let inputElementB = null;
				let inputToFindA = "highSeverityInput";
				let inputToFindB = "lowSeverityInput";
				inputsArray.forEach((input) => {
												if(input.id == inputToFindA){inputElementA = input;}
												if(input.id == inputToFindB){inputElementB = input;}  
												});
			...
			it('\nSimulate selecting items of high severity', function(){
				...
				//There should be no high severity items
				expect(ReactDOM.findDOMNode(renderedBugList.refs.bugItemNum0)).toEqual(null);
...
describe('Testing ordering functions', function(){
	
	
	var renderedBugList;
	var sortBarReference;
	
	
			//create fresh instance of buglist, as to ensure tests run independantly
			beforeEach(function() {
				
				renderedBugList = TestUtils.renderIntoDocument(
					<BugList />
				);				
				sortBarReference = renderedBugList.refs.ColumnSortHeaderComponent;
			});
		
			//clear instances of buglist and inputs to ensure integrity of following tests
			afterEach(function() {
				renderedBugList = null;
				sortBarReference = null;
			});	

			it('\nSimulate selecting to sort by ID ascending', function(){

				let sortingButton = sortBarReference.refs.buttonID;
				
				TestUtils.Simulate.click(sortingButton);
				...
				componentReference = renderedBugList.refs.bugItemNum5;
				elementReference = componentReference.refs.bugID;
			...
			it('\nSimulate selecting to sort by ID desccending', function(){
